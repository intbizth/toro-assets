...
$(document).ready(function () {
    ...
    $('.toggle-nav').click(function() {
        ...
        $('.mobile-nav').toggleClass('header-nav-opened');
    ...
    $('.mobile-nav .accordion-toggle').click(function(e) {
    ...
    $('.mobile-nav .menu-item-has-children > a').click(function(e) {
        e.preventDefault();
        $(this).siblings('.accordion-toggle').trigger('click');
    });


    var $nav = $('#nav');
    var $headerNav = $('.header-nav-top');
    ...
    if ($nav.length) {
        ...
        $(window).scroll(function () {
            ...
            if (y > posTop) {
                $('#navigate-block').css("top", -posTop+"px");    $nav.addClass('scroll__nav');
                $headerNav.addClass('scroll__nav-top');
                $('.logo-top').addClass('scroll__logo-top');
                $('.logo-scroll').addClass('scroll__logo-scroll');
                $('.social-header').addClass('social-header__scroll');
            }
            else {
                $('#navigate-block').css("top", -y+"px");   $nav.removeClass('scroll__nav');
                $headerNav.removeClass('scroll__nav-top');
                $('.logo-top').removeClass('scroll__logo-top');
                $('.logo-scroll').removeClass('scroll__logo-scroll');
                $('.social-header').removeClass('social-header__scroll');
            }
        });
    }
    //----- end Menu System ------//

    var startNode = $('.live-score__top-bar--date').length-1;

    var currentMatchDay = $('#match_day').data('current-match-day');
    if(currentMatchDay) {
        startNode = $('#md-' + currentMatchDay).index();
    }

    var timer = '';
    var container = $('.live-score');
    $('#match_day').slick({
        dots: false,
        infinite: false,
        speed: 300,
        ...
        variableWidth: true,
        centerMode: true,
        initialSlide: startNode
    })
        .on('init', function(event, slick){
            console.log('slider was initialized', $(this));
        })
        .on('beforeChange', function(event, slick, currentSlide, nextSlide){
            console.log('before change/curret', currentSlide);
        })
        .on('afterChange', function(event, slick, currentSlide, nextSlide){
            var th = $('.live-score__top-bar--date:eq('+currentSlide+')');
            var data = {"match_day": th.data('match-day')};
            if(timer != '') {
                clearTimeout(timer);
            }
            timer = setTimeout(function(){
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        container.removeClass('ajax-loading');
                        th.closest('.live-score').find('.match-by-day-container').html(response);
                    }
                });
            }, 1000);
    ...
    $('.schedule-container__item').slick({
        dots: true,
        infinite: false,
        speed: 300,
        slideToShow: 1,
    ...
    $('.next-slide').click(function (e) {
        $('.schedule-container__item').slick('slickNext');
    });
    ...
    $('.slick-slider-container--has-child').slick({
        arrows: false,
        dots: false,
        infinite: false,
        slidesToShow: 1,
    ...
    $(".jumbo-content, .main-featured__tab").swipe({
        excludedElements:".noSwipe",
        swipeRight: function(){
            var slick = $(this).closest('.slick-slider-container--has-child').length ? $(this).closest('.slick-slider-container--has-child')
            : $(this).siblings('.slick-slider-container--has-child');

            var index = slick.slick('slickCurrentSlide');
            if(index > 0){
                slick.slick('slickPrev');

                toggleSlideTab($('.main-featured__tab a.active'), 'prev');
            }
        ...
        swipeLeft:function(){
            ...
            if(index < slick.find('.tab-pane').length - 1){
                slick.slick('slickNext');

                toggleSlideTab($('.main-featured__tab a.active'), 'next');
            }

            return;
        }
    });
    $('.main-featured__tab a').swipe({
        excludedElements:".noSwipe",
        tap: function(e){
            console.log('tap');
            e.preventDefault();

            var slick = $('.main-featured__tab').siblings('.slick-slider-container--has-child');

            slick.slick('slickGoTo', $($(this).attr('href')).data('slick-index'));
    ...
    function toggleSlideTab(ele, side){
        ele.removeClass('active');

        if(side == 'next'){
            ele.parent().next().find('a').addClass('active');
        }else{
            ele.parent().prev().find('a').addClass('active');
        }
    }

    $('.toggle-social').on('click', 'span', function(){
        if($('.toggle-nav span.active').length){
            $('.toggle-nav span').click();
        }
        if(!$(this).hasClass('active')){
            $(this).addClass('active');
            $('#social-feed-mobile').css('left', '0');
        }else{
            $(this).removeClass('active');
            $('#social-feed-mobile').css('left', '100%');
        }
    });
    $('.toggle-nav').on('click', 'span', function(){
        if($('.toggle-social span.active').length){
            $('.toggle-social span').click();
        }
        if(!$(this).hasClass('active')){
            $(this).addClass('active');
            $('#main-menu-mobile').css('left', '0');
        }else{
            $(this).removeClass('active');
            $('#main-menu-mobile').css('left', '-100%');
        }
    });
});
...
(function($) {
    ...
    $.fn.lineup = function(options) {
        return this.each(function() {
            ...
            // if (editor && hidden===null) {
            //     $this.html("<div style='text-align:center'><h3><i class='fa fa-warning'></i>&nbsp;In edit mode - Need 'hiddenId'</h3></div>");
            // }
            ...
            htmlConcat += "<div class=\"col-sm-"+(editor?"6":"12")+"\"><div class=\"field-container\"><div class=\"field\">";
            for (var i = 0; i < 11; i++) {
                htmlConcat += "<ul class='player dragbox player" + i + "' data-slot='" + i + "'></ul>";
            }
            htmlConcat += "</div></div></div>";
            htmlConcat+="<div class=\"col-sm-6 edit-box\"><ul class=\"player-box dragbox player-avatar\">";

            for (var i = 0; i < 11; i++) {
                htmlConcat += "<li id='" + $ID + "_con" + i + "' class='"+(editor?"player-avatar__block":""/*no drag if edit*/)+" playercon'"+(editor ? " draggable='true'" : " ") + "data-id='" + $playerList[i].ID + "' style='background: url(\"" + $playerList[i].image + "\");'><span>" + $playerList[i].name + "</span></li>";
            }
            htmlConcat += "</ul></div>";
            if(debug) {
    ...
    $.fn.lineupArrange = function(options) {
        return this.each(function() {
            ...
            var formation = {
                '4-4-2' : [
                    {"name":"","x":50,"y":92},{"name":"DL","x":16,"y":71},{"name":"DC","x":36,"y":74},{"name":"DC","x":63,"y":74},{"name":"DR","x":84,"y":71},{"name":"ML","x":17,"y":43},{"name":"MC","x":35,"y":50},{"name":"MC","x":65,"y":50},{"name":"MR","x":84,"y":43},{"name":"FC","x":35,"y":23},{"name":"FC","x":65,"y":23}
                ],
                '4-2-3-1' : [{"name":"","x":50,"y":92},{"name":"DL","x":16,"y":71},{"name":"DC","x":36,"y":74},{"name":"DC","x":63,"y":74},{"name":"DR","x":84,"y":71},{"name":"ML","x":20,"y":38},{"name":"MC","x":35,"y":53},{"name":"MC","x":65,"y":53},{"name":"MR","x":80,"y":38},{"name":"FC","x":50,"y":19},{"name":"FC","x":50,"y":34}],
                '4-3-3' : [{"x":50,"y":92},{"x":16,"y":65},{"x":38,"y":74},{"x":61,"y":74},{"x":84,"y":65},{"x":18,"y":27},{"x":50,"y":59},{"x":72,"y":50},{"x":82,"y":27},{"x":50,"y":19},{"x":36,"y":42}],
                '4-2-1-3' : [{"name":"","x":50,"y":92},{"name":"DL","x":16,"y":71},{"name":"DC","x":39,"y":74},{"name":"DC","x":60,"y":74},{"name":"DR","x":84,"y":71},{"name":"ML","x":50,"y":40},{"name":"MC","x":39,"y":59},{"name":"MC","x":61,"y":59},{"name":"MR","x":80,"y":31},{"name":"FC","x":21,"y":31},{"name":"FC","x":50,"y":21}]
            };
    ...
    $.fn.cameraPanel = function(options) {
        return this.each(function() {
            var $this = $(this),
                viewport = document.getElementById( 'field-container' ),
                world = document.getElementById( 'field' ),
                d = 0,
                p = 400,
                worldXAngle = 0,
                worldYAngle = 0,
            ...
            if (hover) {
                ...
                window.addEventListener( 'mousemove', function( e ) {
                    if (rotateStyle=='h') { worldYAngle = -( .5 - ( e.clientX / window.innerWidth ) ) * 180; }
                    else { worldZAngle = ( .5 - ( e.clientX / window.innerHeight ) ) * 180; }
            ...
            function updateView() {
                var t = 'translateZ( ' + d + 'px ) rotateX( ' + worldXAngle + 'deg) rotateY( ' + worldYAngle + 'deg) rotateZ( ' + worldZAngle + 'deg)';

                $('.field').css({'transform': t});
                if (rotateStyle=='h') {
                    var sp = 'translateX(-50%) translateY(-50%) translateZ('+playerFloat+'px) rotateY( ' + (-worldYAngle) + 'deg ) rotateX( ' + ( - worldXAngle ) + 'deg )';
                } else {
                    var sp = 'translateX(-50%) translateY(-50%) translateZ('+playerFloat+'px) rotateZ( ' + (- worldZAngle) + 'deg ) rotateX( ' + ( - worldXAngle ) + 'deg )';
                }
                $('.player').css({'transform':sp});
            }

            function onContainerMouseWheel( event ) {
                event = event ? event : window.event;
                scrollScale = (event.detail ? event.detail * -5 : event.wheelDelta / 8);
                if (d - scrollScale < 390 && d - scrollScale  > -600) {
                    d -= scrollScale;
                }
            ...
            var htmlConcat = "<label class=\"radio-inline\"><input type=\"radio\" name=\"cam\" checked=\"true\" class=\"cam-p\"/>Portial Cam</label><label class=\"radio-inline\"><input type=\"radio\" name=\"cam\" class=\"cam-l\"/>Lean Cam</label><label class=\"radio-inline\">\<input type=\"radio\" name=\"cam\" class=\"cam-s1\"/>Side Cam</label>\
          <label class=\"radio-inline\">\
            <input type=\"radio\" name=\"cam\" class=\"cam-rt\"/>Rotate Cam</label>";
            $this.html(htmlConcat);
            var interval;
            //--cam--
